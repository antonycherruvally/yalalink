...
class Education_model extends CI_Model{
	...
	public function search( $data ) {
		...
		$mode = $data['searcheducation'];
		unset($data['searcheducation']);
		$data = $string->cleanArray( $data );

		$db->select('*', FALSE);
		$db->from('tbl_postings AS a');
		$db->join('tbl_educations AS b', 'a.id = b.post_id', 'LEFT');
	...
	public function prepare( $data ) {
		$string = Core::getHelper('str')->setLinker('education/view/');
	...
	function getListings($limit = NULL, $offset = NULL,$type = NULL){
		...
		if($type){
			...
			$this->db->where('a.type','Universities');
		}
		$query = $this->db->where(array('a.main_category'=>'Education','status !='=>4,'status !='=>0,'a.country'=>$c_code->id))->limit($limit,$offset)->order_by('a.id', 'desc')->get('tbl_postings as a');
	...
	function getListingsCount($type = NULL){
		...
		$count = $this->db->where(array('a.main_category'=>'Education','status !='=>4,'status !='=>0,'a.country'=>$c_code->id))->from('tbl_postings as a')->count_all_results();
		return $count;
	}
	...
	function getDetails($id = NULL){
        $this->db->select('a.*,b.post_id,b.school_type,b.curriculum,b.inspection_rating,b.availability,b.education_status,b.principal, b.mobile as e_mobile, b.email as e_email',FALSE);
		$this->db->join('tbl_educations AS b', 'a.id = b.post_id', 'left');
