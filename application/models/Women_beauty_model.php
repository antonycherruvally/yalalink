...
class Women_beauty_model extends CI_Model{
	...
	public function search( $data ) {
		...
		$mode = $data['searchwomenbeauty'];
		unset($data['searchwomenbeauty']);
		$data = $string->cleanArray( $data );


		$db->select('*', FALSE);
		$db->from('tbl_postings AS a');
		$db->join('tbl_womenbeauty AS b', 'a.id = b.post_id', 'LEFT');
	...
	/**
	 *	Get Women and Beauty
	 */
	public function get( $data ) {
		$string = Core::getHelper('str');
		unset($data['womenbeauty']);
		$page = isset($data['page']) ? $data['page'] : 0;
		$type = isset($data['purpose']) ? $data['purpose'] : 'Women';

		$lists = $this->getListings(1000, $page, $type);
	...
	public function prepare( $data ) {
		$string = Core::getHelper('str')->setLinker('women-beauty/view/');
	...
	function getListings($limit = NULL, $offset = NULL,$type = NULL, $category = NULL){
		...
		if($type){
			...
			$this->db->where('a.type','Women');
		}
		if($category){
			$this->db->where('b.category',$category);
		}
		$query = $this->db->where(array('a.main_category'=>'Women & Beauty','status !='=>4,'status !='=>0,'a.country'=>$c_code->id))->limit($limit,$offset)->order_by('a.id', 'desc')->get('tbl_postings as a');
	...
	function getListingsCount($type = NULL, $category = NULL){
		...
		$count = $this->db->where(array('a.main_category'=>'Women & Beauty','status !='=>4,'status !='=>0,'a.country'=>$c_code->id))->from('tbl_postings as a')->count_all_results();
		return $count;
	}
	...
	function getDetails($id = NULL){
        $this->db->select('a.*,b.post_id, b.phone, b.fax, b.email as w_email, b.website',FALSE);
		$this->db->join('tbl_womenbeauty AS b', 'a.id = b.post_id', 'left');
