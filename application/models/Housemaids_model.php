...
class Housemaids_model extends CI_Model{
	...
	public function search( $data ) {
		...
		$mode = $data['searchhousemaids'];
		unset($data['searchhousemaids']);
		$data = $string->cleanArray( $data );

		$db->select('*', FALSE);
		$db->from('tbl_postings AS a');
		$db->join('tbl_house_maids AS b', 'a.id = b.post_id', 'LEFT');
	...
	public function prepare( $data ) {
		$string = Core::getHelper('str')->setLinker('housemaids/view/');
	...
	function getListings($limit = NULL, $offset = NULL,$type = NULL){
		...
		if($type){
			...
			$this->db->where('a.type','Domestic');
		}
		$query = $this->db->where(array('a.main_category'=>'House Maids','status !='=>4,'status !='=>0,'a.country'=>$c_code->id))->limit($limit,$offset)->order_by('a.id', 'desc')->get('tbl_postings as a');
	...
	function getListingsCount($type = NULL){
		...
		$count = $this->db->where(array('a.main_category'=>'House Maids','status !='=>4,'status !='=>0,'a.country'=>$c_code->id))->from('tbl_postings as a')->count_all_results();
		return $count;
	}
