...
class Electronics extends CI_Controller{
	...
	public function index(){
		...
		$data->hotdeals = $this->yalalink_model->DealsPostsPage('Electronics');
		$data->special = $this->yalalink_model->SpecialPostsPage('Electronics');
		$_SESSION["dcountries"] = array();
		$data->countrylatest = $this->yalalink_model->countrylatest('Electronics', 1, 0, 1);
		$data->countrylatest1 = $this->yalalink_model->countrylatest('Electronics', 1, 0, 1);
		$data->countrylatest2 = $this->yalalink_model->countrylatest('Electronics', 1, 0, 1);
		$this->load->view('electro/electronics/main', $data);
		// $this->load->view('front_end/electronics', $data);
	}
	public function getListings(){ 
		...
		$data['results'] = $this->electronics_model->getListings($config['per_page'], $offset, $type, $category);
		...
		foreach($results as $val){
			...
			if($val->featured){ 
			  ...
			  if($check){ 
			   ...
			   if($val->title_ar!='' || $val->title_ar == 'NULL'){ 
			  	  $ar = '<a href="electronics/view/'.@$latest.'" class="ar-txt add-ar card-ar">'.'...'.substr($val->title_ar,0,40).'</a>';
			  }else{
				  $ar = '';
			  }
			  if($val->title_en){
				  if($val->title_ar!='' || $val->title_ar == 'NULL'){ 
			  	  	  $en = '<a href="electronics/view/'.@$latest.'">'.substr($val->title_en,0,25).'</a>';
				  }else{
					  $en = '<a href="electronics/view/'.@$latest.'">'.substr($val->title_en,0,25).'</a>';
				  }
			  }else{
			...
			<p class="card-text small"> '.$en.$ar.'</p>
              ...
              <a href="electronics/view/'.@$latest.'"><img class="card-img-top" src="'.$image.'" alt="'.substr($val->title_en,0,100).'"></a>
              <div class="card-body">
	...
	public function getListingslist(){ 
      ...
      <div class="col-md-6"> 
			 ...
			 <a href="electronics/view/'.@$latest.'"><img class="card-img-top-list" src="'.$image.'" alt="'.substr($val->title_en,0,100).'"></a></div>
                  <div class="col-md-6"> 
				  <div class="card-body">
                   
                     <p class="card-text small-list"> '.$en.$ar.'</p>
				   
                    <h5 class="card-title3">'.$currency.' '.number_format(@$val->price).'</h5>
	...
	public function view($category){
			...
			$data->view = $this->electronics_model->getDetails($pid);
			$data->postdate = $this->auto_model->getPostDate($pid);
			$data->user = $this->yalalink_model->getUserDetails($data->view->user_id);
			$data->images = $this->electronics_model->getImages($pid);
			$data->title = $data->view->title_en.' - Electronics in Dubai, Sharjah, Abu Dhabi, Ajman, Ras Al Khaimah ';
			$data->meta_desc =  '';
			...
			$data->page_title = 'Electronics';
			$data->meta_url = 'electronics/view/'.$category;
			$data->hotdeals = $ads->getHotDeals('Electronics', 6);
		    $data->special = $ads->getSpecialAds('Electronics', 6);
			$this->load->view('front_end/electronics-details',$data);
	}
	function paginationStyling($type, $category){
        ...
        $config['base_url'] = base_url().'/electronics?type='.$type.'&category='.$category;
		$config['total_rows'] = $this->electronics_model->getListingsCount($type, $category);
