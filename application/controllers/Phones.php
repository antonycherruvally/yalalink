...
class Phones extends CI_Controller{
	...
	public function index(){
		...
		$data->hotdeals = $this->yalalink_model->DealsPostsPage('Phones');
		$data->special = $this->yalalink_model->SpecialPostsPage('Phones');
		$_SESSION["dcountries"] = array();
		$data->countrylatest = $this->yalalink_model->countrylatest('Phones', 1);
		$data->countrylatest1 = $this->yalalink_model->countrylatest('Phones', 1);
		$data->countrylatest2 = $this->yalalink_model->countrylatest('Phones', 1);
		$this->load->view('electro/phones/main', $data);
		// $this->load->view('front_end/phones', $data);
	}
	public function getListings(){ 
		...
		$data['results'] = $this->phones_model->getListings($config['per_page'], $offset, $type);
		...
		foreach($results as $val){
			...
			if($val->featured){ 
			  ...
			  if($check){ 
			   ...
			   if($val->title_ar!='' || $val->title_ar == 'NULL'){ 
			  	  $ar = '<a href="phones/view/'.@$latest.'" class="ar-txt add-ar card-ar">'.'...'.substr($val->title_ar,0,40).'</a>';
			  }else{
				  $ar = '';
			  }
			  if($val->title_en){
				  if($val->title_ar!='' || $val->title_ar == 'NULL'){ 
			  	  	  $en = '<a href="phones/view/'.@$latest.'">'.substr($val->title_en,0,35).'...</a>';
				  }else{
					  $en = '<a href="phones/view/'.@$latest.'">'.substr($val->title_en,0,35).'...</a>';
				  }
			  }else{
			...
			<p class="card-text small"> '.$en.$ar.'</p>
              ...
              <a href="phones/view/'.@$latest.'"><img class="card-img-top" src="'.$image.'" alt="'.substr($val->title_en,0,100).'"></a>
              <div class="card-body">
              ...
              <div class="bottom">
                <div class="rating"> '.$favorites.' </div>
					...
					<span class ="year" style="color: black;font-size: 13px;"><a href="phones/view/'.@$latest.'" title="Year" style="color: #00000096;"><i class="fas fa-calendar" aria-hidden="true" style="font-size: 17px;" alt="bedroom">  </i></a>'.@$val->release_year.'  </span>
                    <span class ="kilometer" style="color: black;font-size: 13px;"><a href="phones/view/'.@$latest.'" title="kilometer" style="color: #00000096;"><i class="fas fa-tachometer-alt" aria-hidden="true" style="font-size: 17px;" alt="bedroom">  </i></a>'.@$val->color.'  </span> <span class="added-date">'.date("j, F Y", strtotime($val->added_date)).'</span> </div>
            </div>';
			if($val->featured){
            $result .= '</div>';
			}
	...
	public function getListingslist(){ 
	  ...
	  <p class="card-text small-list"> '.$en.$ar.'</p>
			 ...
			 <a href="phones/view/'.@$latest.'"><img class="card-img-top-list" src="'.$image.'" alt="'.substr($val->title_en,0,100).'"></a></div>
                  <div class="col-md-6"> 
				  <div class="card-body">
	...
	public function getListingslistsmall(){ 
		...
		foreach($results as $val){
			...
			if($val->featured){ 
			  ...
			  if($val->title_en){
				  if($val->title_ar!='' || $val->title_ar == 'NULL'){ 
			  	  	  $en = '<a href="phones/view/'.@$latest.'">'.substr($val->title_en,0,30).'...</a>';
				  }else{
					  $en = '<a href="phones/view/'.@$latest.'">'.substr($val->title_en,0,30).'...</a>';
				  }
			  }else{
	...
	public function view($category){
			...
			$data->page_title = 'Phones';
			$data->meta_url = 'phones/view/'.$category;
			$data->hotdeals = $ads->getHotDeals('Phones', 6);
		    $data->special = $ads->getSpecialAds('Phones', 6);
			$this->load->view('front_end/phone-details',$data);
	}
	function paginationStyling($type){
        ...
        $config['base_url'] = base_url().'/phones?type='.$type;
		$config['total_rows'] = $this->phones_model->getListingsCount($type);
