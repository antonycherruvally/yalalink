...
class Services extends CI_Controller{
	...
	public function index(){
		...
		$data->hotdeals = $this->yalalink_model->DealsPostsPage('Services');
		$data->special = $this->yalalink_model->SpecialPostsPage('Services');
		$this->load->view('electro/services/main', $data);
	...
	public function getListings(){ 
		...
		$data['results'] = $this->services_model->getListings($config['per_page'], $offset, $type, $category);
		...
		foreach($results as $val){
			  ...
			  $latest = $title.'-'.$val->id;
			   $main_image = $this->services_model->getMainImage($val->id);
			...
			if($val->featured){ 
			  ...
			  if($check){ 
			   ...
			   if($val->title_ar!='' || $val->title_ar == 'NULL'){ 
			  	  $ar = '<a href="services/view/'.@$latest.'" class="ar-txt add-ar card-ar">'.'...'.substr($val->title_ar,0,40).'</a>';
			  }else{
				  $ar = '';
			  }
			  if($val->title_en){
				  if($val->title_ar!='' || $val->title_ar == 'NULL'){ 
			  	  	  $en = '<a href="services/view/'.@$latest.'">'.substr($val->title_en,0,15).'...</a>';
				  }else{
					  $en = '<a href="services/view/'.@$latest.'">'.substr($val->title_en,0,15).'...</a>';
				  }
			  }else{
			...
			<p class="card-text small"> '.$en.$ar.'</p>
              ...
              <a href="services/view/'.@$latest.'"><img class="card-img-top" src="'.$image.'" alt="'.substr($val->title_en,0,100).'"></a>
              <div class="card-body">
	...
	public function getListingslist(){ 
	  ...
	  <p class="card-text small-list"> '.$en.$ar.'</p>
			 ...
			 <a href="services/view/'.@$latest.'"><img class="card-img-top-list" src="'.$image.'" alt="'.substr($val->title_en,0,100).'"></a></div>
                  <div class="col-md-6"> 
				  <div class="card-body">
                   
                   <p class="card-text-small"></p>
                    <h5 class="card-title2">Description</h5>
	...
	public function view($category){
			...
			$data->view = $this->services_model->getDetails($pid);
			$data->user = $this->yalalink_model->getUserDetails($data->view->user_id);
			$data->images = $this->services_model->getImages($pid);
			$data->title = $data->view->title_en.' - Services in Dubai, Sharjah, Abu Dhabi, Ajman, Ras Al Khaimah ';
			$data->meta_desc =  '';
			...
			$data->page_title = 'Services';
			$data->meta_url = 'services/view/'.$category;
			$data->hotdeals = $ads->getHotDeals('Services', 6);
		    $data->special = $ads->getSpecialAds('Services', 6);
			$this->load->view('front_end/services-details',$data);
	}
	function paginationStyling($type, $category){
        ...
        $config['base_url'] = base_url().'/services?type='.$type.'&category='.$category;
		$config['total_rows'] = $this->services_model->getListingsCount($type, $category);
