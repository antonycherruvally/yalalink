...
class Real_estate extends CI_Controller{
	...
	public function index(){
		...
		$data->hotdeals = $ads->getHotDeals('Real Estate', 6);
		$data->special = $ads->getSpecialAds('Real Estate', 6);
		$this->load->view('electro/realestate/main', $data);
	}
      ...
      <div class="row">';
		foreach($results as $val){
			...
			if($val->featured){ 
			  ...
			  if($val->title_en){
			  	  if(($country_code == 'AE' || $country_code == 'OM' || $country_code == 'SA' || $country_code == 'EG') && ($val->title_ar!='' || $val->title_ar == 'NULL')){ 
			  	  	  $en = '<a href="real-estate_new/view/'.@$latest.'">'.substr($val->title_en,0,40).'...</a>';
				  }else{
					  $en = '<a href="real-estate_new/view/'.@$latest.'">'.substr($val->title_en,0,40).'...</a>';
				  }
			  }else{
	...
	public function getListingslist(){ 
		...
		$type = $this->input->get('type');
		//$property_type = $this->input->get('property-type');
		//$property_type = $this->session->property($property_type);
	$prop = $_SESSION["property"];
	  ...
	  <p class="card-text small-list"> '.$en.$ar.'</p>
			 ...
			 <a href="real-estate/view/'.@$latest.'"><img class="card-img-top-grid" src="'.$image.'" alt="'.substr($val->title_en,0,100).'"></a></div>
			 <div class="col-md-6">
              <div class="card-body">
               
               
                <h5 class="card-title2">'.$currency.' '.number_format(@$val->price).'</h5>
                <p class="card-text-list-main">'.substr(strip_tags($val->description),0,500).'...'.'</p>
              </div>'.$show_user.'
      ...
      <div class="row">';
		foreach($results as $val){
			...
			if($val->featured){ 
			  ...
			  if($val->title_en){
			  	  if(($country_code == 'AE' || $country_code == 'OM' || $country_code == 'SA' || $country_code == 'EG') && ($val->title_ar!='' || $val->title_ar == 'NULL')){ 
			  	  	  $en = '<a href="real-estate_new/view/'.@$latest.'">'.substr($val->title_en,0,65).'...</a>';
				  }else{
					  $en = '<a href="real-estate_new/view/'.@$latest.'">'.substr($val->title_en,0,65).'...</a>';
				  }
			  }else{
			...
			<a href="auto/view/'.@$latest.'"><img class="card-img-top-list" src="'.$image.'" alt="'.substr($val->title_en,0,100).'"></a></div>
