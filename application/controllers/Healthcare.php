...
class Healthcare extends CI_Controller{
	...
	public function index(){
		$data = new stdClass();
		$data->page_title = 'Healthcare';
		$data->hotdeals = $this->yalalink_model->DealsPostsPage('Health Care');
		$data->special = $this->yalalink_model->SpecialPostsPage('Health Care');
		$this->load->view('electro/healthcare/main', $data);
		// $this->load->view('front_end/healthcare', $data);
	}
	public function getListings(){ 
		...
		$data['results'] = $this->healthcare_model->getListings($config['per_page'], $offset, $type);
		...
		foreach($results as $val){
			  ...
			  $latest = $title.'-'.$val->id;
			   $main_image = $this->healthcare_model->getMainImage($val->id);
			...
			if($val->featured){ 
			  ...
			  if($check){ 
			   ...
			   if($val->title_ar!='' || $val->title_ar == 'NULL'){ 
			  	  $ar = '<a href="healthcare/view/'.@$latest.'" class="ar-txt add-ar card-ar">'.'...'.substr($val->title_ar,0,30).'</a>';
			  }else{
				  $ar = '';
			  }
			  if($val->title_en){
				  if($val->title_ar!='' || $val->title_ar == 'NULL'){ 
			  	  	  $en = '<a href="healthcare/view/'.@$latest.'">'.substr($val->title_en,0,45).'...</a>';
				  }else{
					  $en = '<a href="healthcare/view/'.@$latest.'">'.substr($val->title_en,0,45).'...</a>';
				  }
			  }else{
			...
			<p class="card-text small"> '.$en.$ar.'</p>
              ...
              <a href="healthcare/view/'.@$latest.'"><img class="card-img-top" src="'.$image.'" alt="'.substr($val->title_en,0,100).'"></a>
              <div class="card-body">
	...
	public function getListingslist(){ 
	  ...
	  <p class="card-text small-list"> '.$en.$ar.'</p>
			 ...
			 <a href="healthcare/view/'.@$latest.'"><img class="card-img-top-list" src="'.$image.'" alt="'.substr($val->title_en,0,100).'"></a></div>
                  <div class="col-md-6"> 
				  <div class="card-body">
	...
	public function getListingslistsmall(){ 
		...
		foreach($results as $val){
			...
			if($val->featured){ 
			  ...
			  if($val->title_en){
				  if($val->title_ar!='' || $val->title_ar == 'NULL'){ 
			  	  	  $en = '<a href="healthcare/view/'.@$latest.'">'.substr($val->title_en,0,60).'...</a>';
				  }else{
					  $en = '<a href="healthcare/view/'.@$latest.'">'.substr($val->title_en,0,60).'...</a>';
				  }
			  }else{
	...
	public function view($category){
			...
			$data->view = $this->healthcare_model->getDetails($pid);
			$data->user = $this->yalalink_model->getUserDetails($data->view->user_id);
			$data->images = $this->healthcare_model->getImages($pid);
			$data->title = $data->view->title_en.' - Healthcare in Dubai, Sharjah, Abu Dhabi, Ajman, Ras Al Khaimah ';
			$data->meta_desc =  '';
			...
			$data->page_title = 'Healthcare';
			$data->meta_url = 'healthcare/view/'.$category;
			$data->hotdeals = $this->yalalink_model->DealsPostsPage('Health Care');
		    $data->special = $this->yalalink_model->SpecialPostsPage('Health Care');
			$this->load->view('front_end/healthcare-details',$data);
	}
	function paginationStyling($type){
        ...
        $config['base_url'] = base_url().'/healthcare?type='.$type;
		$config['total_rows'] = $this->healthcare_model->getListingsCount($type);
