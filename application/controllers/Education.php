...
class Education extends CI_Controller{
	...
	public function index(){
		...
		$data->hotdeals = $this->yalalink_model->DealsPostsPage('Education');
		$data->special = $this->yalalink_model->SpecialPostsPage('Education');
		$this->load->view('electro/education/main', $data);
		// $this->load->view('front_end/education', $data);
	}
	public function getListings(){ 
		...
		$data['results'] = $this->education_model->getListings($config['per_page'], $offset, $type);
		...
		foreach($results as $val){
			  ...
			  $latest = $title.'-'.$val->id;
			   $main_image = $this->education_model->getMainImage($val->id);
			...
			if($val->featured){ 
			  ...
			  if($check){ 
			   ...
			   if($val->title_ar!='' || $val->title_ar == 'NULL'){ 
			  	  $ar = '<a href="education/view/'.@$latest.'" class="ar-txt add-ar card-ar">'.'...'.substr($val->title_ar,0,40).'</a>';
			  }else{
				  $ar = '';
			  }
			  if($val->title_en){
				  if($val->title_ar!='' || $val->title_ar == 'NULL'){ 
			  	  	  $en = '<a href="education/view/'.@$latest.'">'.substr($val->title_en,0,15).'...</a>';
				  }else{
					  $en = '<a href="education/view/'.@$latest.'">'.substr($val->title_en,0,25).'...</a>';
				  }
			  }else{
				  $en = '';
			  }
			...
			<p class="card-text small"> '.$en.$ar.'</p>
              ...
              <a href="education/view/'.@$latest.'"><img class="card-img-top" src="'.$image.'" alt="Card image cap"></a>
              <div class="card-body">
	...
	public function getListingslist(){ 
	  ...
	  <p class="card-text small-list"> '.$en.$ar.'</p>
			 ...
			 <a href="education/view/'.@$latest.'"><img class="card-img-top-list" src="'.$image.'" alt="'.substr($val->title_en,0,100).'"></a></div>
                  <div class="col-md-6"> 
				  <div class="card-body">
                   
                   <p class="card-text-small"></p>
                    <h5 class="card-title2">Details</h5>
	...
	public function view($category){
			...
			$data->view = $this->education_model->getDetails($pid);
			$data->user = $this->yalalink_model->getUserDetails($data->view->user_id);
			$data->images = $this->education_model->getImages($pid);
			...
			$data->page_title = 'Education';
			$data->meta_url = 'education/view/'.$category;
			$data->hotdeals = $this->yalalink_model->DealsPostsPage('Education');
		    $data->special = $this->yalalink_model->SpecialPostsPage('Education');
			$this->load->view('front_end/education-details',$data);
	}
	function paginationStyling($type){
        ...
        $config['base_url'] = base_url().'/education?type='.$type;
		$config['total_rows'] = $this->education_model->getListingsCount($type);
