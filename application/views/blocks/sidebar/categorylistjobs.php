				...
				$act = 'active';
					foreach( $cats as $ct ) {
						...
						<li class="nav-item mainpage categorywid" style="height: 28px;">
							<a class="nav-link <?= $act ?> category" data-toggle="tab" id="<?= $ct->id ?>" href="javascript:void(0)" role="tab" style="text-align: justify;	"><?= empty($ct->subcategory) ? $ct->category:$ct->subcategory ?><span style="float:right;padding-right: 36px;"><?= empty($ct->ar_subcategory) ? $ct->ar_category:$ct->ar_subcategory ?></span></a>
						</li>
						<?php
						$act = "";
					}
...
">
					...
					foreach( $cats as $ct ) {
						...
						?>
						 <div class="col-sm-<?php echo $bootstrapColWidth; ?> col-xs-<?php echo $bootstrapColWidth; ?> ">
							<div class="sidenewbtn nav-link <?= $act ?> category" data-toggle="tab" id="<?= $ct->id ?>" href="javascript:void(0)" role="tab" style="    width: 100%;
    font-size: 10px;
    ...
    if($rowCount % $numOfCols == 0) echo '</div><div class="row nextrow" >';
		...
		<div class="filterbtn filterbtnjob">Filter بحث متقدم
</div>
</aside>