        ...
        <div class="row row-lg">
          <div class="col-md-12"> 
            ...
            <div class="example-wrap">
              <h4 class="example-title">Add New User</h4>
              <?php if($this->session->flashdata('message')){ ?>
              <?php echo $this->session->flashdata('message'); ?>
              <?php } ?>
              <div class="example">
                <form class="form-signin cv-form" action="admin/insertUser" enctype="multipart/form-data" method="post" name="users_form" id="users_form">
                  <div class="row">
                    <div class="col-md-3">
                  ...
                  <div class="form-group form-material">
                    <button type="submit" class="btn btn-success ladda-button" data-style="zoom-in" data-plugin="ladda" data-type="progress" id="submit_users">Save</button>
...
$(document).ready(function(e) {
	<?php /*?>$(document).on('click', '#submit_users', function(e) {
        $.ajax({
            'url': '<?php echo base_url(); ?>admin/validateEmail/' + $("#email").val(),
            'async': false,
            'type': "get",
            'dataType': "html",
            'success': function(data) {
                if (data != 'empty') {
                    $('.email-exist').fadeIn('slow');
					$("#submit_users").prop('disabled', true);
                } else {
                    $('.email-exist').hide();
					$("#submit_users").prop('disabled', false);
                }
            }
        });
    });
	$('#confirm_email').on('keyup', function() {
        $.ajax({
            'url': '<?php echo base_url(); ?>admin/validateEmail/' + this.value,
            'async': false,
            'type': "get",
            'dataType': "html",
            'success': function(data) {
